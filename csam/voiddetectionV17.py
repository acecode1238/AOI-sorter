...
def find_white_in_black_shapes(roi_gray):
    ...
    for cnt in contours:
        ...
        white_pixels_contour = np.sum(np.logical_and(mask == 255, binary_white == 255))
        total_pixels_contour = cv2.countNonZero(mask)

        x, y, w, h = cv2.boundingRect(cnt)
        
        all_areas = [cv2.contourArea(c) for c in contours if cv2.contourArea(c) > 100]
        if all_areas:
            avg_area = np.mean(all_areas)
        print("Contour area:", area, "Expected:", avg_area)

# ---- Normal single chip continues below ----
        if w > roi_gray.shape[1] * 0.95 or h > roi_gray.shape[0] * 0.95:
         continue


        if x == 0 or y == 0 or x + w == roi_gray.shape[1] or y + h == roi_gray.shape[0]:
            continue
        bbox_mask = np.zeros(roi_gray.shape, dtype=np.uint8)
        cv2.rectangle(bbox_mask, (x, y), (x + w, y + h), 255, -1)

        white_pixels_bbox = np.sum(np.logical_and(bbox_mask == 255, binary_white == 255))
        total_pixels_bbox = cv2.countNonZero(bbox_mask)

        if total_pixels_contour == 0 or total_pixels_bbox == 0:
            continue

        aspect_ratio = w / float(h)
        shape_type = "Square" if 0.85 <= aspect_ratio <= 1.15 else "Rectangle/Other"
        ...
        results.append({
            ...
            "white_pixels_bbox": int(white_pixels_bbox),
            "total_pixels_bbox": int(total_pixels_bbox),
            "percent_white_bbox": round(100 * white_pixels_bbox / total_pixels_bbox, 3),
            "black_pixels_bbox": int(total_pixels_bbox - white_pixels_bbox),
...
def main1():
    ...
    while True:
        ...
        if key == ord('+') or key == ord('='):
            box_speed = min(box_speed + 1, 50)
        elif key == ord('-'):
            box_speed = max(box_speed - 1, 1)
        if key == ord(']'):
